...
const Hints = styled.button`
  ...
  ${props => props.display === 'visible' && 'transform: translateX(0); opacity: 1; '}
...
function Quiz() {
  ...
  const [playCorrectAnswer] = useSound('../src/sounds/676401__cjspellsfish__score-1.mp3', {
    volume: 0.1,
  });
  ...
  return (
    <QuizLayout>
      ...
      <Row gap="1.5rem" align="center" justify="space-between" wrap="wrap">
        <Button
          onClick={() => {
            onClearQuiz();
            navigate('/');
        ...
        <Row gap="0.5rem">
          ...
          {showHint && (
            <HintsModal ref={ref}>
              ...
              <IoCloseOutline onClick={() => setShowHint(false)} />
            </HintsModal>
          )}
      ...
      <Container display={display}>
        <Question display={display}>{currentQuestion.questionText}</Question>
        ...
        <ChoiceList display={display}>
          {answers?.map((answer, index) => (
            <Answer answer={answer} key={answer.id} optionLetter={optionLetters.at(index)} />
          ))}
        </ChoiceList>
